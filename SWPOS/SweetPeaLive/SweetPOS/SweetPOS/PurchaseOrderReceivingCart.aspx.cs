...
namespace SweetPOS
{
    public partial class PurchaseOrderReceivingCart : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Session["currentPage"] = "PurchaseOrderReceivingCart.aspx";
            try
            {
                ...
                else
                {
                    ...
                    if (!Page.IsPostBack)
                    {
                        purchaseOrder = POM.ReturnPurchaseOrderCurrent(Convert.ToInt32(Request.QueryString["purchO"].ToString()), CU)[0];
                        purchaseOrder.intTransactionTypeID = 8;
        ...
        protected void grdVendorSupplierItems_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                double poCost = 0;
                string poCostAtPurchase = ((TextBox)grdVendorSupplierItems.Rows[index].Cells[4].FindControl("txtCostAtPurchase")).Text.Replace("$", "");
                if (poCostAtPurchase != "")
                {
                    if (double.TryParse(poCostAtPurchase, out poCost))
                    {
                        poCost = Convert.ToDouble(poCostAtPurchase);
                    }
                }
                int poQuantity = 1;
                string poQuantityAtPurchase = ((TextBox)grdVendorSupplierItems.Rows[index].Cells[4].FindControl("txtQuantity")).Text;
                ...
                PurchaseOrderItem selectedSku = new PurchaseOrderItem();
                foreach (PurchaseOrderItem poi in purchaseOrder.lstPurchaseOrderItem)
                {
                    if (poi.intPurchaseOrderItemID == Convert.ToInt32(e.CommandArgument.ToString()))
                    {
                        selectedSku = poi;
                    }
                }
                selectedSku.fltReceivedCost = poCost;
                selectedSku.intReceivedQuantity = poQuantity;
                POM.UpdateItemIntoPurchaseOrderCartReceiving(selectedSku, CU.terminal.intBusinessNumber);
                UpdatePurchaseOrderTotal();
            }
            //Exception catch
            catch (ThreadAbortException) { }
        ...
        protected void grdPurchaseOrderItems_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                POM.ResetReceivingPurchaseOrderItem(selectedSku, CU);
                UpdatePurchaseOrderTotal();
            }
            //Exception catch
            catch (ThreadAbortException) { }
        ...
        private void UpdatePurchaseOrderTotal()
        {
            ...
            try
            {
                ...
                purchaseOrder = POM.ReturnReceivingPurchaseOrder(Convert.ToInt32(Request.QueryString["purchO"].ToString()), Convert.ToDateTime(CUM.ConvertDate(DateTime.Now.ToLocalTime())), CU)[0];

                grdVendorSupplierItems.DataSource = purchaseOrder.lstPurchaseOrderItem;
        ...
        protected void btnHoldPurchaseOrderForProcessing_Click(object sender, EventArgs e)
        {
            //Collects current method for error tracking
            string strMethod = "btnHoldPurchaseOrderForProcessing_Click";
            try
            {
                //Changes page to the Purchases Home page
                POM.ResetReceivingPurchaseOrder(purchaseOrder, CU);
                Response.Redirect("PurchaseOrderHomePage.aspx", true);
            }
            //Exception catch
            catch (ThreadAbortException) { }
        ...
        protected void btnFinalizeReceiving_Click(object sender, EventArgs e)
        {
            //Collects current method for error tracking
            string strMethod = "btnFinalizeReceiving_Click";
            try
            {
                if (RCM.CheckForItemsInTransaction(purchaseOrder))
                {
                    //finalize po save item into a new fully received table.
                    POM.FinalizePurchaseOrder(purchaseOrder, Convert.ToDateTime(CUM.ConvertDate(DateTime.Now.ToLocalTime())), CU.terminal.intBusinessNumber);
                    var nameValues = HttpUtility.ParseQueryString(Request.QueryString.ToString());
                    nameValues.Set("purchO", purchaseOrder.intPurchaseOrderID.ToString());
                    Response.Redirect("PrintablePurchaseOrder.aspx?" + nameValues, true);
                }
                else
                {
