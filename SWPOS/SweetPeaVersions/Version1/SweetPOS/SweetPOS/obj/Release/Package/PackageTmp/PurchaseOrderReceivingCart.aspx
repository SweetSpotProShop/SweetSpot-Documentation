<%@ Page Title="" Language="C#" MasterPageFile="~/SweetPOSMasterPage.Master" AutoEventWireup="true" CodeBehind="PurchaseOrderReceivingCart.aspx.cs" Inherits="SweetPOS.PurchaseOrderReceivingCart" %>
...
<asp:Content ID="PurchaseOrderCartPageContent" ContentPlaceHolderID="IndividualPageContent" runat="server">
	<div id="PurchaseOrderCart">
		<asp:panel id="pnlDefaultButton" runat="server" defaultbutton="btnHoldPurchaseOrderForProcessing">
			<asp:Label ID="lblVendor" runat="server" Text="Vendor:" />
			<asp:TextBox ID="txtVendorDisplay" runat="server" Text="" AutoComplete="off" Enable="false" />
			<br />
			<br />
			...
			<asp:GridView ID="grdVendorSupplierItems" runat="server" AutoGenerateColumns="false" OnRowCommand="grdVendorSupplierItems_RowCommand" RowStyle-HorizontalAlign="Center">
				<Columns>
					<asp:TemplateField HeaderText="Add To PO">
						<ItemTemplate>
							<asp:LinkButton ID="lkbAddItemToPO" Text="Add Item" CommandArgument='<%#Eval("intPurchaseOrderItemID") %>' runat="server" CausesValidation="false" />
						</ItemTemplate>
					</asp:TemplateField>
					...
					<asp:TemplateField HeaderText="Quantity">
						<ItemTemplate>
							<div>
								<asp:TextBox ID="txtQuantity" runat="server" AutoComplete="off" Text='<%# Convert.ToInt32(Eval("intPurchaseOrderQuantity")) - Convert.ToInt32(Eval("intReceivedQuantity")) %>' />
							</div>
						</ItemTemplate>
					</asp:TemplateField>
					...
					<asp:TemplateField HeaderText="Cost">
						<ItemTemplate>
							<div>
								<asp:TextBox ID="txtCostAtPurchase" runat="server" AutoComplete="off" Text='<%# Eval("fltPurchaseOrderCost") %>' />
							</div>
						</ItemTemplate>
					</asp:TemplateField>
				</Columns>
			...
			<asp:GridView ID="grdPurchaseOrderItems" runat="server" AutoGenerateColumns="false" OnRowCommand="grdPurchaseOrderItems_RowCommand" RowStyle-HorizontalAlign="Center">
				<Columns>
					...
					<asp:BoundField DataField="varSku" ReadOnly="true" HeaderText="SKU" />
					<asp:BoundField DataField="varVendorSku" ReadOnly="true" HeaderText="Vendor SKU" />
					<asp:BoundField DataField="intReceivedQuantity" ReadOnly="true" HeaderText="Quantity" />
					<asp:BoundField DataField="varItemDescription" ReadOnly="true" HeaderText="Description" />
					<asp:BoundField DataField="fltReceivedCost" ReadOnly="true" HeaderText="Cost" DataFormatString="{0:C}" />
				</Columns>
			</asp:GridView>
			<hr />
			...
			<asp:Button ID="btnHoldPurchaseOrderForProcessing" runat="server" Text="Hold Purchase Order" OnClick="btnHoldPurchaseOrderForProcessing_Click" CausesValidation="false" />
			<asp:Button ID="btnFinalizeReceiving" runat="server" Text="Receive Purchase Order" OnClick="btnFinalizeReceiving_Click" CausesValidation="false" />
		</asp:panel>
	</div>
</asp:Content>