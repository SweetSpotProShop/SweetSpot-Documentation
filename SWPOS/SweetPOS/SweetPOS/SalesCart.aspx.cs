                ...
                else
                {
                    ...
                    if (!RM.ItemAlreadyInCart(selectedItem, CU.terminal.intBusinessNumber))
                    {
                        ...
                        if (selectedItem.intInventoryID == RM.ReturnTradeInSkuForLocation(CU)[0].intInventoryID)
                        {
                            ...
                            Inventory newTrade = IM.ReturnInventoryItem(RM.AddTradeInToInventoryTable(selectedItem, Convert.ToDateTime(CUM.ConvertDate(DateTime.Now.ToLocalTime())), CU), CU.terminal.intBusinessNumber, Convert.ToDateTime(CUM.ConvertDate(DateTime.Now.ToLocalTime())))[0];
